...
const root = protobufHelper.loadSync([
  path.join( __dirname, "bettinghouse.proto"),
  path.join( __dirname, "domain.proto")
], allIncludeDirs);

const ItemAdded = root.lookupType("org.example.bettinghouse.domain.CashInAdded");
const Bet = root.lookupType("org.example.bettinghouse.domain.Bet");

// Start server
const bettingDealEntity = require("./bettinghouse.js"); 
const cloudstate = require("cloudstate");
const server = new cloudstate.CloudState();


server.addEntity(bettingDealEntity);
...
function callAndInit(snapshot) {
  console.log("heeeeyyeyeyeyeye")
  const call = eventSourcedClient.handle();
  call.write({
    init: {
      serviceName: "org.example.bettinghouse.BettingHouseService",
      entityKey: "123",
      snapshot: snapshot
    }
  });
...
function getBet(call) {
  return sendCommand(call, "GetBet", root.lookupType("org.example.bettinghouse.GetBet").create({
    userId: "123"
  }));
}

function addCashIn(call, item) {
  return sendCommand(call, "AddCashIn", root.lookupType("org.example.shoppingcart.AddLineCashIn").create(item));
}
